    ...
    try:
        if extension.lower() == ".docx":
            ...
            try:
                ...
                logging.error(
                    "PyPDF2 library is not installed. Please install it to process PDF files: pip install PyPDF2"
                )
                return None
            ...
            with open(filepath, "rb") as f:
                ...
                for page_num in range(len(reader.pages)):
                    ...
                    try:
                        ...
                        logging.warning(
                            f"Could not extract text from page {page_num+1} of {filepath}: {page_e}"
                        )
        ...
        if "PyPDF2" in str(type(e)):  # Check if it's a PyPDF2 error
            logging.error(
                f"Could not read PDF (possibly corrupted or password protected): {filepath} - {e}"
            )
            return None
...
def construct_full_prompt(student_text, master_prompt_template, scenario_text=None):
    ...
    if placeholder not in master_prompt_template:
        logging.error(
            f"Placeholder '{placeholder}' not found in draft prompt template."
        )
        return (
...
def save_draft_feedback_to_docx(feedback_prose, output_filepath, student_identifier):
    ...
    try:
        ...
        # Split the feedback prose by newlines and add with simple formatting
        # We apply basic Markdown-like heuristics for clearer structure
        for line in feedback_prose.splitlines():
            stripped = line.strip()

            # Empty line becomes a blank paragraph for spacing
            if not stripped:
                doc.add_paragraph()
                continue

            # Markdown style headings using leading '#'
            heading_match = re.match(r"^(#{1,6})\s*(.+)", stripped)
            if heading_match:
                level = len(heading_match.group(1)) + 1  # offset so # -> lvl2
        ...
        logging.error(
            f"Failed to create DOCX draft report for {student_identifier}: {e}"
        )
...
def main():
    ...
    for i, filename in enumerate(assessment_files):
        ...
        logging.info(
            f"--- Processing file ({i+1}/{len(assessment_files)}): {filename} ---"
        )
        processed_files += 1
        ...
        if word_count < 50:  # Arbitrary threshold
            logging.warning(
                f"Extracted text for {filename} is very short ({word_count} words). Feedback might be limited."
            )
        ...
        if scenario_text:
            ...
            logging.info(
                "No specific scenario detected; proceeding without scenario context."
            )

        full_prompt = construct_full_prompt(
            extracted_text, draft_prompt_template, scenario_text
        )
        ...
        if not ai_feedback_prose:
            logging.warning(
                f"Skipping {filename} due to Gemini API call failure or empty response."
            )
            continue

        output_filename_base = student_identifier
        output_docx_path = os.path.join(
            OUTPUT_FOLDER, f"{output_filename_base}_draft_feedback.docx"
        )

        save_draft_feedback_to_docx(
            ai_feedback_prose, output_docx_path, student_identifier
        )
        ...
        if (
            USE_API_DELAY and i < len(assessment_files) - 1
        ):  # Don't sleep after the last file
            logging.info(
                f"Waiting for {API_DELAY_SECONDS} seconds before processing next file..."
            )
    ...
    logging.info(
        f"Successfully generated draft feedback for: {successful_feedback_generations} files"
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        logging.warning(
            "PyPDF2 library is not installed. PDF files will not be processed. Run 'pip install PyPDF2' to enable PDF support."
        )
